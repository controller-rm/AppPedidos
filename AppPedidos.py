...
# =====================================================
# HELPERS DE UI ESTILO APP (lista de produtos / carrinho)
# =====================================================
_ICONES_PRODUTO = [
    (("CANECA", "XICARA"), "☕"),
    (("BULE",), "🫖"),
    (("PRATO", "TRAVESSA"), "🍽️"),
    (("BOWL", "TIGELA", "SOPEIRA", "CONSOME"), "🥣"),
    (("VASO",), "🏺"),
    (("CESTA",), "🧺"),
    (("ACUCAREIRO", "LEITEIRA", "POTE"), "🫙"),
]


def icone_produto(descricao: str) -> str:
    """Escolhe um emoji ilustrativo com base em palavras-chave da descrição."""
    desc = (descricao or "").upper()
    for chaves, emoji in _ICONES_PRODUTO:
        if any(chave in desc for chave in chaves):
            return emoji
    return "📦"


def _dec_qtd(key_qtd: str) -> None:
    st.session_state[key_qtd] = max(1, st.session_state.get(key_qtd, 1) - 1)


def _inc_qtd(key_qtd: str) -> None:
    st.session_state[key_qtd] = st.session_state.get(key_qtd, 1) + 1


def _adicionar_ao_carrinho(codigo: str, descricao: str, preco: float, key_qtd: str) -> None:
    ...
    if codigo in carrinho_map:
        idx = next(
            i for i, item in enumerate(st.session_state.carrinho) if item["codigo"] == codigo
        )
...
_novo_pedido_row = st.container()
with _novo_pedido_row:
    st.markdown('<div class="novo-pedido-row-marker"></div>', unsafe_allow_html=True)
    _col_novo_spacer, _col_novo_btn = st.columns([4, 1])
    with _col_novo_btn:
...
/* =====================================================
   BOTÃO "+ NOVO" COMPACTO (canto superior direito, estilo app nativo)
   ===================================================== */
.novo-pedido-row-marker { display: none; }
...
/* =====================================================
   LISTA ESTILO APP (PRODUTOS / CARRINHO)
   Cartão único, ícone circular, título + meta, preço à direita
   e stepper (− qtd +) — inspirado em apps de força de venda.
   ===================================================== */
.list-row-marker { display: none; }

div[data-testid="stVerticalBlock"]:has(> div > .list-row-marker) {
...
/* Stepper (− qtd +) */
div[data-testid="stVerticalBlock"]:has(> div > .list-row-marker) div[data-testid="stButton"] > button {
...
div[data-testid="stVerticalBlock"]:has(> div > .list-row-marker) button[kind="secondary"] {
    background: #ffffff !important;
    color: var(--zionne-green-900) !important;
    border: 1.5px solid var(--zionne-green) !important;
}

.remove-btn-marker { display: none; }

div[data-testid="stVerticalBlock"]:has(> div > .remove-btn-marker) button[kind="secondary"] {
...
/* Selo (pill) de status, ex.: "No pedido" */
.pill-badge {
    display: inline-block;
    font-size: 11.5px;
    font-weight: 700;
...
/* Barra de ferramentas simples acima da lista (contagem de itens) */
.list-toolbar {
    font-size: 12.5px;
    color: var(--muted);
...
/* =====================================================
   BARRA DE TOTAL (estilo faixa "TOTAL" de apps de pedido)
   ===================================================== */
.total-box {
    background: #eaf4ee;
    padding: 14px 16px;
...
/* =====================================================
   BARRA INFERIOR DE AÇÕES (ícone + rótulo, estilo bottom nav)
   ===================================================== */
.bottom-action-bar-marker { display: none; }
...
/* =====================================================
   ZIONNE UI 2026 — acabamento conforme mockup de referência
   ===================================================== */
[data-testid="stAppViewContainer"] > .main .block-container {
    max-width: 1180px;
    padding-top: 1.1rem;
...
.app-bar {
    ...
    margin-bottom: 0;
    padding: 17px 24px;
    box-shadow: 0 10px 28px rgba(13,63,30,.18);
}
.app-bar-title { font-size: 25px; }
.app-bar-subtitle { font-size: 14px; }
.app-bar-cart {
    background: rgba(0,0,0,.14);
    border: 1px solid rgba(255,255,255,.12);
}

/* Novo pedido encaixado visualmente no cabeçalho */
...
div[data-testid="stVerticalBlock"]:has(> div > .novo-pedido-row-marker) div[data-testid="stButton"] > button {
    background: rgba(4,74,31,.92) !important;
    color: #fff !important;
...
div[data-baseweb="tab"][aria-selected="true"] {
    background: linear-gradient(180deg,#d6b34f 0%,#c69b27 100%) !important;
    box-shadow: 0 4px 10px rgba(201,163,93,.28);
}
div[data-baseweb="tab"] > button,
div[data-baseweb="tab"] > div {
    border-radius: 999px !important;
}
div[data-baseweb="tab"] button[aria-selected="true"] { border-bottom: none !important; }
...
/* Card de cliente como faixa verde do mockup */
.cliente-card {
    background: linear-gradient(135deg,#e9f6eb 0%,#d9efdd 100%);
    border: 2px solid #176b34;
...
.cliente-card-badge {
    ...
    color: #fff !important;
    font-size: 16px;
    background: linear-gradient(90deg,#0d4d24,#218d42);
}
.cliente-card-nome { font-size: 16px; }
.cliente-card-linha { font-size: 13px; }

/* Produtos: cartão creme com faixa verde */
.product-grid-marker { display:none; }
div[data-testid="stVerticalBlock"]:has(> div > .product-grid-marker) > div[data-testid="stVerticalBlockBorderWrapper"],
div[data-testid="stVerticalBlock"]:has(> div > .product-grid-marker) [data-testid="stVerticalBlockBorderWrapper"] {
    background: linear-gradient(145deg,#fffdf7 0%,#f7efd9 100%) !important;
    border: 1px solid #eee4c8 !important;
    border-left: 7px solid #15933d !important;
    min-height: 205px;
}
.product-sku {
    display:inline-block; background:#ece9e2; border-radius:999px;
    padding:4px 9px; font-size:11px; font-weight:700; margin-bottom:8px;
}
.product-title { font-size:15px; font-weight:900; line-height:1.1; min-height:34px; }
.product-price { color:#11642d !important; font-size:19px; font-weight:900; margin:7px 0 5px; }
.product-status { font-size:12px; margin-top:7px; color:#25352b !important; }
.product-status-dot { color:#1b9b42 !important; }

/* Botões verdes mais próximos da referência */
button[kind="primary"] {
    background: linear-gradient(180deg,#15933d 0%,#08772c 100%) !important;
}
button[kind="primary"]:hover {
    background: linear-gradient(180deg,#118536 0%,#066a27 100%) !important;
}

@media (max-width: 700px) {
    [data-testid="stAppViewContainer"] > .main .block-container { padding-left:.55rem; padding-right:.55rem; }
    .app-bar { padding:14px 15px; }
    .app-bar-title { font-size:20px; }
    .app-bar-cart { display:none; }
    div[data-testid="stVerticalBlock"]:has(> div > .novo-pedido-row-marker) { margin-top:-49px; padding-right:10px; }
        ...
        for inicio in range(0, len(produtos), 2):
            ...
            for pos, row in enumerate(produtos[inicio:inicio + 2]):
                with grid_cols[pos]:
                    ...
                    key_qtd = f"stepper_{codigo}_{rc}"
                    if key_qtd not in st.session_state:
                        st.session_state[key_qtd] = 1

                    card = st.container(border=True)
                    with card:
                        st.markdown('<div class="product-grid-marker"></div>', unsafe_allow_html=True)
                        st.markdown(
                            f'<span class="product-sku">📦 SKU {codigo}</span>'
                            f'<div class="product-title">{descricao}</div>'
                            f'<div class="product-price">R$ {preco:,.2f}</div>',
                            unsafe_allow_html=True,
                        )
                        ...
                        with col_add:
                            st.button(
                                "Adicionar +", key=f"add_{codigo}_{rc}", type="primary",
                                use_container_width=True, on_click=_adicionar_ao_carrinho,
                                args=(codigo, descricao, preco, key_qtd),
                            )

                        if ja_no_carrinho:
                            qtd_total = carrinho_map[codigo]["qtd"]
                            st.markdown(
                                f'<div class="product-status"><span class="product-status-dot">●</span> No pedido: {qtd_total} unidade(s)</div>',
                                unsafe_allow_html=True,
                            )
                        else:
...
with tab_carrinho:
    ...
    if st.session_state.carrinho and st.session_state.dados_cliente:
        ...
        st.markdown(
            f"<div class='list-toolbar'><b>{len(st.session_state.carrinho)}</b> item(ns) no pedido</div>",
            unsafe_allow_html=True,
        )
...
with tab_final:
    ...
    if st.session_state.pedido_gerado:
        ...
        st.markdown("#### 📤 Compartilhar Pedido")

        telefone_limpo = somente_digitos(pedido["telefone_cliente"])
        telefone_zionne_limpo = somente_digitos(pedido["telefone_zionne"])

        if not telefone_limpo:
            st.warning("Informe o telefone do cliente para habilitar o envio por WhatsApp.")
        if not telefone_zionne_limpo:
            st.warning("Informe o Telefone WhatsApp Zionne para habilitar o envio interno.")

        action_bar = st.container()
        with action_bar:
            st.markdown('<div class="bottom-action-bar-marker"></div>', unsafe_allow_html=True)
            c_csv, c_pdf, c_wpp_cli, c_wpp_zio = st.columns(4)

            with c_csv:
                st.download_button(
                    "⬇️ CSV", pedido["csv_content"], pedido["csv_nome"],
                    mime="text/csv", use_container_width=True,
                )
            with c_pdf:
                st.download_button(
                    "⬇️ PDF", pedido["pdf_bytes"], pedido["pdf_nome"],
                    mime="application/pdf", use_container_width=True,
                )
            with c_wpp_cli:
                if telefone_limpo:
                    link_cliente = f"https://wa.me/55{telefone_limpo}?text={quote(pedido['mensagem_cliente'])}"
                    st.link_button("📲 Cliente", link_cliente, use_container_width=True)
            with c_wpp_zio:
                if telefone_zionne_limpo:
                    link_zionne = f"https://wa.me/55{telefone_zionne_limpo}?text={quote(pedido['mensagem_zionne'])}"
                    st.link_button("📲 Zionne", link_zionne, use_container_width=True)
